...
The application launches in fullscreen mode (`fullscreen` branch), mapping the internal grid to your monitor's resolution. The start node is rendered in green, and the goal node is rendered in orange. The calculated path appears as a continuous red line circumventing the light blue polygon boundaries.

Press the Escape key to close the application and return to your desktop safely.
